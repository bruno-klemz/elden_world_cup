...
- **Progresso:** lista de IDs de bosses derrotados em `shared_preferences` (chave-valor simples).
...
- **Pendente:** a mesma arte renderizada **borrada e em preto-e-branco** + ícone 🔒 + nome embaixo (teaser). O usuário sabe qual boss é, mas a arte só "desbloqueia" colorida ao derrotar.
...
- **Seções roláveis**, nesta ordem:
  1. **📍 Onde encontrar** — preview do mapa (ver §7) + nome do local.
...
Marcar como derrotado dispara a **animação de revelação** na figurinha correspondente (borrado P&B → colorido).
...
- **`domain/`** (unit) — marcar/desmarcar boss; contadores por região; percentual geral; imutabilidade dos modelos.
- **`data/`** (unit) — parse de `bosses.json` (incluindo campos opcionais e enums de dano); round-trip de leitura/escrita do progresso no `shared_preferences` (com fake/in-memory).
- **`presentation/`** (widget) — `StickerSlot` nos dois estados (pendente borrado vs. derrotado colorido); botão de ação da bottom sheet alternando entre marcar/desmarcar; colapso do `SliverAppBar`.
